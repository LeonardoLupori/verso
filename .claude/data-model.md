...
| `original_path` | str | Absolute path to the full-resolution source image. For multi-scene container formats (CZI/LIF) several sections share one path, distinguished by `scene_index`. |
| `thumbnail_path` | str | Path to the working-resolution OME-TIFF (relative or absolute). |
| `scene_index` | int | Index of the scene/image within a multi-scene container file (CZI/LIF). `0` for single-image formats and the first scene of a container. Defaults to `0` for pre-existing project files. |
