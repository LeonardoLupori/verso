...
def test_markers_to_control_points():
    ...
    cps = _markers_to_control_points(markers, overlay_width=400, overlay_height=200)

    assert len(cps) == 1
    cp = cps[0]
    assert abs(cp.src_x - 200.0) < 1e-6
    assert abs(cp.src_y - 50.0) < 1e-6
    assert abs(cp.dst_x - 240.0) < 1e-6
    assert abs(cp.dst_y - 40.0) < 1e-6


def test_control_points_to_markers():
    cps = [ControlPoint(src_x=200.0, src_y=50.0, dst_x=240.0, dst_y=40.0)]
    markers = _control_points_to_markers(cps, overlay_width=400, overlay_height=200)
...
def test_marker_control_point_round_trip():
    ...
    cps = _markers_to_control_points(original, 456, 320)
    restored = _control_points_to_markers(cps, 456, 320)
...
def test_load_visualign_control_points(tmp_path: Path):
    ...
    project = load_visualign(p, overlay_width=456, overlay_height=320)
...
def test_visualign_save_load_round_trip(tmp_path: Path):
    ...
    ow, oh = 456, 320
    project = load_visualign(src, overlay_width=ow, overlay_height=oh)

    dst = tmp_path / "va_out.json"
    save_visualign(project, dst, overlay_width=ow, overlay_height=oh)

    reloaded = load_visualign(dst, overlay_width=ow, overlay_height=oh)
